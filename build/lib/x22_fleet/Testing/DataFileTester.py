...
class DataFileTester:
    ...
    def evaluate_sessions(self, files, tolerance=60, min_timestamp=None):
        ...
        for i, group in enumerate(grouped):
            ...
            files_in_group = [file for file in files if int(re.search(r".*-(\d+)_rec\.bd(?:\.uploaded)?$", file).group(1)) in group]
...
def main():
    ...
    work_locally = True  # Change to False to use FTP mode
    ...
    if files:
        ...
        print(f"Detailed analysis for all sessions has been saved to {analysis_pickle_path}")
