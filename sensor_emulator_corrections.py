...
class SensorEmulator:
    ...
    def build_packet(self):
        ...
        header_samplenumber = self.last_samplenumber
        ...
        payload += struct.pack("<I", header_samplenumber)  # 4 bytes (little-endian)
        ...
        for i in range(num_samples):
            ...
            sample_number = header_samplenumber + i
            sample_timestamp = sample_number / self.fs
            phase1 = (sample_timestamp / self.sine_period) * 2 * math.pi 
            phase2 = phase1 + 2 * math.pi / 3
            phase3 = phase2 + 2 * math.pi / 3            
            # Use the sample index (or sample_number) to generate a phase.
            ...
            acc_x = int(self.acc_amplitude * math.sin(phase1))
            acc_y = int(self.acc_amplitude * math.sin(phase2))
            acc_z = int(self.acc_amplitude * math.sin(phase3))

            gyro_x = int(self.gyro_amplitude * math.sin(phase1))
            gyro_y = int(self.gyro_amplitude * math.sin(phase2))
            gyro_z = int(self.gyro_amplitude * math.sin(phase3))

            mag_x = int(self.mag_amplitude * math.sin(phase1))
            mag_y = int(self.mag_amplitude * math.sin(phase2))
            mag_z = int(self.mag_amplitude * math.sin(phase3))
        ...
        self.last_samplenumber = header_samplenumber + num_samples
...
def publish_loop(emulator, client, topic, delay=0.1, send_threshold=300):
    databuffer = []
    try:
        while True:
            packet = emulator.build_packet()
            databuffer.extend(packet)  # Append new packet to buffer
            
            if len(databuffer) >= send_threshold:  # Check if buffer exceeds threshold
                result = client.publish(topic, bytes(databuffer), qos=1)
                status = result[0]

                if status == 0:
                    print(
                        f"Published {len(databuffer)} bytes ({len(databuffer) // emulator.num_samples_per_packet} packets); "
                        f"base timestamp: {emulator.last_samplenumber}"
                    )
                    databuffer.clear()  # Clear buffer after successful publish
                else:
...
def main():
    ...
    client = connect_mqtt()
    client.loop_start()
    publish_loop(emulator, client, TOPIC,delay = emulator.num_samples_per_packet / emulator.fs)
    client.loop_stop()
    client.disconnect()
